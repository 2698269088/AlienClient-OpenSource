...
public class FontRenderer implements Closeable {
    ...
    public static @NotNull Identifier randomIdentifier() {
        return Identifier.of("alienclient", "temp/" + randomString());
    }
    ...
    private void sizeCheck() {
        ...
        if (gs != this.previousGameScale) {
            ...
            if (secondFont != null) {
                this.secondFont = secondFont.deriveFont(originalSize * this.scaleMul);
            }
        }
    }
    ...
    private GlyphMap generateMap(char from, char to) {
        GlyphMap gm;
        if (secondFont != null) {
            gm = new GlyphMap(from, to, this.font, this.secondFont, randomIdentifier(), padding);
        } else {
            gm = new GlyphMap(from, to, this.font, randomIdentifier(), padding);
        }
        maps.add(gm);
        return gm;
    }
    ...
    public void drawString(MatrixStack stack, String s, double x, double y, int color, boolean shadow) {
        ...
        float b = ((color) & 0xff) / 255f;
        float a = ((color >> 24) & 0xff) / 255f;
        drawString(stack, s, (float) x, (float) y, r, g, b, a, shadow);
    }

    public void drawString(MatrixStack stack, String s, double x, double y, Color color, boolean shadow) {
        drawString(stack, s, (float) x, (float) y, color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha(), shadow);
    }
    ...
    public void drawStringWithShadow(MatrixStack stack, String s, double x, double y, int color) {
        ...
        float b = ((color) & 0xff) / 255f;
        float a = ((color >> 24) & 0xff) / 255f;
        drawStringWithShadow(stack, s, (float) x, (float) y, r, g, b, a);
    }

    public void drawStringWithShadow(MatrixStack stack, String s, double x, double y, Color color) {
        drawStringWithShadow(stack, s, (float) x, (float) y, color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha());
    }

    public void drawStringWithShadow(MatrixStack stack, String s, float x, float y, float r, float g, float b, float a) {
        drawString(stack, s, x, y, r, g, b, a, true);
    }
            ...
            for (int i = 0; i < chars.length; i++) {
                ...
                if (glyph != null) {
                    if (glyph.value() != ' ') {
                        ...
                        if (shadow) {
                            DrawEntry shadowEntry = new DrawEntry(xOffset + 1, yOffset + 1, 0, 0, 0, glyph);
                            GLYPH_PAGE_CACHE.computeIfAbsent(i1, integer -> new ObjectArrayList<>()).add(shadowEntry);
                        }
    ...
    public void drawCenteredString(MatrixStack stack, String s, double x, double y, int color) {
        ...
        float b = ((color) & 0xff) / 255f;
        float a = ((color >> 24) & 0xff) / 255f;
        drawStringWithShadow(stack, s, (float) (x - getWidth(s) / 2f), (float) y, r, g, b, a);
    }

    public void drawCenteredString(MatrixStack stack, String s, double x, double y, Color color) {
        drawStringWithShadow(stack, s, (float) (x - getWidth(s) / 2f), (float) y, color.getRed() / 255f, color.getGreen() / 255f, color.getBlue() / 255f, color.getAlpha() / 255f);
    }

    public void drawCenteredString(MatrixStack stack, String s, float x, float y, float r, float g, float b, float a) {
        drawStringWithShadow(stack, s, x - getWidth(s) / 2f, y, r, g, b, a);
    }
    ...
    public void drawGradientString(MatrixStack stack, String s, float x, float y) {
        drawString(stack, s, x, y, 255, 255, 255, 255);
    }

    public void drawGradientCenteredString(MatrixStack matrices, String s, float x, float y) {
        drawGradientString(matrices, s, x - getWidth(s) / 2f, y);
    }
