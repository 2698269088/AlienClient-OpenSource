...
public class TimerModule extends Module {
    ...
    public final BindSetting boostKey = add(new BindSetting("HoldKey", -1));
    public final SliderSetting boost = add(new SliderSetting("OnKey", 1, 0.1, 10, 0.01));
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        timer.setMs(Math.min(Math.max(0, timer.getMs()), accumulate.getValueInt()));
        if (MovementUtil.isMoving() && !EntityUtil.isInsideBlock()) {
            ...
            if (moving) {
                ...
                if (reset.getValue()) {
                    ...
                    timer.setMs(Math.max(lastMs - timer2.getMs(), 0));
                }
                moving = false;
            }
            end.setLength(timer.getMs());
            end.reset();
        }

        if (indicator.getValue()) {
            double current = (moving ? (Math.max(lastMs - timer2.getMs(), 0)) : timer.getMs());
