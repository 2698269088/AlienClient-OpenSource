...
public class BindCommand extends Command {
    ...
    public void runCommand(String[] parameters) {
        ...
        if (module == null) {
            sendChatMessage("§4Unknown module!");
            return;
        }
        if (parameters.length == 1) {
            sendChatMessage("§fPlease specify a §bkey§f.");
            return;
        }
        String rkey = parameters[1];
        if (rkey == null) {
            sendChatMessage("§4Unknown Error");
            return;
        }
        if (module.setBind(rkey.toUpperCase())) {
            sendChatMessage("§fBind for §r" + module.getName() + "§f set to §r" + rkey.toUpperCase());
        }
    }
