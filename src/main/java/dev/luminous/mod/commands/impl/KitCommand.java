...
public class KitCommand extends Command {
                    ...
                    try {
                        for (File file : Manager.getFolder().listFiles()) {
                            if (file.getName().endsWith(".kit")) {
                                ...
                                sendChatMessage("Kit: [" + name.substring(0, name.length() - 4) + "]");
                            }
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
            ...
            case "create" -> {
                if (parameters.length == 2) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < 36; ++i) {
                            printwriter.println(i + ":" + mc.player.getInventory().getStack(i).getItem().getTranslationKey());
                        }
                        printwriter.close();
                        sendChatMessage("§fKit [" + parameters[1] + "]" + " created");
                    } catch (Exception e) {
                        sendChatMessage("§fKit [" + parameters[1] + "]" + " create failed");
                        e.printStackTrace();
                    }
                    return;
                }
            ...
            case "delete" -> {
                if (parameters.length == 2) {
                    try {
                        ...
                        sendChatMessage("§fKit [" + parameters[1] + "]" + " removed");
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    return;
    ...
    public String[] getAutocorrect(int count, List<String> seperated) {
        if (count == 1) {
            ...
            for (String x : list) {
                if (input.equalsIgnoreCase(Alien.getPrefix() + "kit") || x.toLowerCase().startsWith(input)) {
                    correct.add(x);
                }
            }
